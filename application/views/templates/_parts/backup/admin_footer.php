...
<script type="text/javascript">
  jQuery(document).ready(function() {
    ...
    function load_data(page, dataFor){
      $.ajax({
        url: "<?php echo base_url(); ?>admin/dash/infoByPagination/"+page,
        method: "POST",
        data: { infoFor: dataFor },
        dataType:"json",
        success:function(data){
          $(data.tab).html(data.information);
          $(data.pagination).html(data.paginationLink);
    ...
    $(document).on("click", ".pagination li a", function(event){
      ...
      var collectDataFor = $(this).attr("href");
      var splitdataFor = collectDataFor.split("/");
      var dataFor = splitdataFor[1];
      load_data(page, dataFor);
    });

    function load_db(){
      $.ajax({
        url: "<?php echo base_url(); ?>admin/database/backup_db/",
        method: "POST",
        success:function(data){
          window.location.href = '<?php echo base_url(); ?>admin/database/';
        }
      });
    }
    ...
    function load_app(){
      $.ajax({
        url: "<?php echo base_url(); ?>admin/database/backup_app/",
        method: "POST",
        success:function(data){
          window.location.href = '<?php echo base_url(); ?>admin/database/';
        }
    ...
    function load_system(){
      $.ajax({
        url: "<?php echo base_url(); ?>admin/database/backup_full_application/",
        method: "POST",
        success:function(data){
          window.location.href = '<?php echo base_url(); ?>admin/database/';
        }
    ...
    $(document).on("click", ".backup_db", function(event){
      event.preventDefault();
      load_db();
    });

    $(document).on("click", ".backup_app", function(event){
      event.preventDefault();
      load_app();
    });

    $(document).on("click", ".backup_system", function(event){
      event.preventDefault();
      load_system();
    });
